## iOS录制音视频（大文件）计算hash256（SHA256）

这是一个iOS录制音视频计算hash256（SHA256）值的demo，适用于大文件快速计算hash256值（其实核心就是计算大文件hash256，不单单可以用于音视频文件，其他大文件也行，这里以音视频为例）。以下是简要介绍，具体还请看demo源码。如有不对的地方欢迎指正，如有帮助欢迎给star，谢谢！
