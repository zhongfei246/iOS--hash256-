...
- (void)setUpVideo
{
    ...
    //设置输出数据格式  lizhongfei 10.27
    self.videoOutput.videoSettings = @{(__bridge id)kCVPixelBufferPixelFormatTypeKey: @(kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange)};
...
- (void)setUpPreviewLayerWithType:(FMVideoViewType )type
{
    ...
    switch (type) {
        case Type1X1:
            rect = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width);
            break;
        case Type4X3:
            rect = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.width*4/3);
            break;
        case TypeFullScreen:
...
- (void)setUpWriter
{
    ...
    self.writeManager = [[AVAssetWriteManager alloc] initWithURL:self.videoUrl viewType:_viewType andPath:[self createVideoFilePath]];
